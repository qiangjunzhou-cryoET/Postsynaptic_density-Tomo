...
% Nearest distance between cluster pixels
for e = 1:length(t)
    for f = 1:length(t)   
    [index,D_temp]=knnsearch(N(N(:,3) ==f,1:2),N(N(:,3) ==e,1:2));
    Z=sort(D_temp); D(f,1)=Z(1);
    end
X=sort(D);
Dnn(e,1)=X(2);
end
...
%write files
function write(area,Dnn,file)
filename = 'DBSCAN_nearest_distance.xlsx';
title = ["Area_Values","", "Dnn Values"];
...
writematrix(Dnn, filename,'Sheet',sheetName,'Range',['C',num2str(2)]);
